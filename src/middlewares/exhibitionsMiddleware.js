...
const exhibitionsMiddleware = (store) => (next) => (action) => {
  switch (action.type) {
    case FETCH_EXHIBITIONS:
      axios.get('http://aurelia-perrier-server.eddi.cloud:8000/api/exhibitions/homepage')
        .then((response) => {
          setTimeout(() => {
    ...
    case FETCH_USER_ARTWORKS:
      axios.get(
        `http://aurelia-perrier-server.eddi.cloud:8000/api/secure/artworks/exhibitions/${action.payload}/profile`,
        {
          headers: {
    ...
    case UPDATE_USER_ARTWORK:
      axios.patch(
        `http://aurelia-perrier-server.eddi.cloud:8000/api/secure/artworks/${action.artworkId}/edit`,
        {
    ...
    case SUBMIT_NEW_ARTWORK:
      axios.post(
        'http://aurelia-perrier-server.eddi.cloud:8000/api/secure/artworks/new',
        {
    ...
    case DELETE_USER_ARTWORK:
      axios.delete(
        `http://aurelia-perrier-server.eddi.cloud:8000/api/secure/artworks/${action.id}/delete`,
        {
          headers: {
            Authorization: `Bearer ${store.getState().users.token}`,
          },
