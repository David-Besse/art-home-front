...
const userInformations = () => {
  ...
  const handleSubmit = (evt) => {
    ...
    if (result.length > 0 && result.includes('email')) {
      ...
      dispatch(
        messageToShow(
          'warning',
          'Votre identifiant a changé, veuillez vous reconnecter avec ce dernier.',
        ),
      );
      setTimeout(() => {
        navigate('/');
  ...
  return (
    <section className="userInformations">
      <h2 className="mt-3 mb-3 text-center justify-content-center userBoxTitle fw-bolder">
        Mon Profil
      </h2>
          ...
          <div className="row g-0">
            <div className="col-md-2 d-flex flex-column align-items-center justify-content-start">
              ...
              {isProfileEditingActivated && (
                <Form.Group>
                  <Form.Control
                    placeholder={avatar !== '' ? avatar : AvatarPicture}
                    type="text"
                    className="mt-2"
                    name="avatar"
                    value={avatar}
            ...
            <div className="col-md-8 my-3 my-md-0">
              <div className="card-body py-0">
                ...
                <p className="card-text fw-bold mb-0">
                  Nom :{' '}
                  {!isProfileEditingActivated && (
                    <span className="fw-normal fst-italic">{lastName}</span>
                  )}
                </p>
                {isProfileEditingActivated && (
                  <Form.Group>
                ...
                <p className="card-text fw-bold mb-0">
                  Prénom :{' '}
                  {!isProfileEditingActivated && (
                    <span className="fw-normal fst-italic">{firstName}</span>
                  )}
                </p>
                {isProfileEditingActivated && (
                  <Form.Group>
                ...
                <p className="card-text fw-bold mb-0">
                  Email :{' '}
                  {!isProfileEditingActivated && (
                    <span className="fw-normal fst-italic">{email}</span>
                  )}
                </p>
                {isProfileEditingActivated && (
                  <Form.Group>
                ...
                {isProfileEditingActivated && (
                  <Form.Group>
                    <Form.Control
                      ...
                      name="birthday"
                      locale="fr-FR"
                      value={`${new Date(birthday).getFullYear()}-${String(new Date(birthday).getMonth() + 1).padStart(2, '0')}-${String(new Date(birthday).getDate()).padStart(2, '0')}`}
                      onChange={(evt) => {
            ...
            <div className="col-md-2 text-end">
              {!isProfileEditingActivated && (
                <Button
                  ...
                  aria-label="activer l'edition des informations de mon profil"
                >
                  <svg
              ...
              {isProfileEditingActivated && (
                <Button
                  ...
                  type="submit"
                  className=""
                  aria-label="valider l'édition de mon profil"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
